...
namespace Devlusion{
    public class Menu : MonoBehaviour{
        ...
        private enum state{ intro, menu, config, loading }
        ...
        void Start(){
            InvokeRepeating("setPosition", 1f, 2f);
        }
            ...
            case state.config:
                ...
                if (Application.platform == RuntimePlatform.WebGLPlayer || Application.platform == RuntimePlatform.Android || Application.platform == RuntimePlatform.IPhonePlayer)
                    ...
                    foreach (Resolution res in resolutions){
                        if (Mathf.Floor(resoluc) == altura)
                            GUI.Label(new Rect(posH, posV / 2 + sizeV, sizeH + 30, sizeV), Vocabulary.vocabulary[14] + res.width + "x" + res.height);
                        altura++;
                    }

                    resoluc = GUI.HorizontalSlider (new Rect (posH, 5 + posV, sizeH + 30, sizeV), resoluc, 0.18F, 10.0F);
                    //if (resoluc != resolucChange){ resolucChange = resoluc;  }
...
/*scrollPosition = GUI.BeginScrollView(new Rect(posH, 5 + posV, sizeH + 30, sizeV), scrollPosition, new Rect(0, 0, Screen.width - 50, Screen.width));
                    if (GUI.Button(new Rect(0, 5 + 0, sizeH / 3, sizeV / 3), "400x300")) {
                        resolutionH = 400;
                        resolutionW = 300;
                        Screen.SetResolution(Screen.resolutions[300].width, Screen.resolutions[400].height, fullscreen, 0);
                    } if (GUI.Button(new Rect(0 + sizeH / 3, 5, sizeH / 3, sizeV / 3), "512x384")){
                        resolutionH = 512;
                        resolutionW = 384;
                       Screen.SetResolution(Screen.resolutions[512].width, Screen.resolutions[512].height, fullscreen, 0);
                    } if (GUI.Button(new Rect(0 + sizeH / 3 * 2, 5, sizeH / 3, sizeV / 3), "640x400")) {
                        resolutionH = 640;
                        resolutionW = 400;
                        Screen.SetResolution(Screen.resolutions[400].width, Screen.resolutions[640].height, fullscreen, 0);
                    }
                    GUI.EndScrollView();
                    */
